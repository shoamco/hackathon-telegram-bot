...
def start(update: Update, context: CallbackContext):
    ...
    logger.info(f"> after location #{chat_id}")
    return PHONE
...
def get_data_user(update, context):
    ...
    library_functions.insert_user(context.user_data['user'])
    user_details = f"user:{context.user_data['user']}\n"
    update.message.reply_text(text=f" Are you a passenger or a driver? ",
                              reply_markup=buttons.get_enter_buttons())
...
start_conv_handler = ConversationHandler(
    entry_points=[CommandHandler('start', start)],
    states={PHONE: [MessageHandler(Filters.contact, get_data_user, edited_updates=True)]},
    fallbacks=[CommandHandler('cancel', cancel)]
)
dispatcher.add_handler(start_conv_handler)
