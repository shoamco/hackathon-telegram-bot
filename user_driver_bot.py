...
def start_driver(update, context):
    ...
    logger.info(f"driver:in start_driver #{chat_id}")
...
def get_place(update, context):
    text = update.message.text
    chat_id = update.effective_chat.id
    logger.info(f"driver:in get_place #{chat_id}")
    context.user_data['ride']['place'] = text
    logger.info(f"driver:ride: {context.user_data['ride']}")
    update.message.reply_text(f"Your ride has been recorded:\n"
                              f"{context.user_data['ride']['date']} - {context.user_data['ride']['time']} "
                              f": {(context.user_data['ride']['source']).title()} -> {(context.user_data['ride']['destination'].title())} ")
                              #f" {context.user_data['ride']['place']} available seats\n")
